...
if not (TEMPLATES_DIR / "index.html").exists():
    raise RuntimeError(f"Template file not found: {TEMPLATES_DIR / 'index.html'}")
...
class WebSocketInputHandler:
    ...
    async def get_input(self, prompt: str, cancellation_token: Optional[object] = None) -> str:
        try:
            ...
            return data.strip() if data else "TERMINATE"
...
Give very brief feedback (max 40 words) on the candidate's answer.
""",
    )
